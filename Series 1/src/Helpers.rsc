...
set[Declaration] getClasses(list[Declaration] asts) {
	set[Declaration] allClasses = {};
	
	visit(asts){
		case c:\class(str name, list[Type] extends, list[Type] implements, list[Declaration] body): allClasses += c;
  		case c:\class(list[Declaration] body): allClasses += c;
	}
